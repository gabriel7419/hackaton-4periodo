...
const InsertUser: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log('Form data to send:', JSON.stringify(formData)); // Logando os dados a serem enviados para verificar o formato
    ...
    try {
      const response = await fetch('http://127.0.0.1:8000/api/register', {
        ...
        headers: {
          ...
          'Accept': 'application/json',        // Adicionando header de aceitação de JSON
        },
      ...
      if (response.ok) {
        const data = await response.json(); // Tentando obter o JSON da resposta
        console.log('Success:', data); // Logando o sucesso da resposta
        setName('');
        setEmail('');
        setRole('admin');
        setPassword('');
        ...
        console.log('Error:', errorData); // Exibindo os dados de erro
